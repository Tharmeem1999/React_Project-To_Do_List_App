...
export const taskReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case "ADD":
            // Create a new array with the current tasks pplus the new one from action.payload
            updatedTasks = [...state, action.payload];
            break;

        // If the action is to remove a task
        case "REMOVE":
            // Filter out the task with the id that matches action.payload
            updatedTasks = state.filter(task => task.id !== action.payload);
            break;

        // If the action is to update a task
        case "UPDATE":
        ...
        // If the action is to sort the tasks by priority
        case "SORT":
        ...
        // If no matching action is found, just return the current state
        default:
            updatedTasks = state;
    }
